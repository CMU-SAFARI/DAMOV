...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    // Memory bypass
    bool bypass = config.get<bool>(prefix + "bypass", false);
    if (bypass){
        latency = 0;
        accLat = 0;
        invLat = 0;
    ...
    if (!isTerminal) {
        if (type == "Simple") {
            ...
            cache = new TimingCache(numLines, cc, array, rp, accLat, invLat, mshrs, tagLat, ways, timingCandidates, domain, bypass, name);
            ...
            cache = new TracingCache(numLines, cc, array, rp, accLat, invLat, traceFile, bypass, name);
        } else {
...
static void PostInitStats(bool perProcessDir, Config& config) {
    ...
    const char* pStatsFile = gm_strdup((pathStr + application + ".zsim.h5").c_str());
    const char* evStatsFile = gm_strdup((pathStr + application + ".zsim-ev.h5").c_str());
    const char* cmpStatsFile = gm_strdup((pathStr + application + ".zsim-cmp.h5").c_str());
