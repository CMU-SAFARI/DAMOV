...
VOID CheckForTermination() {
    ...
    //info("Number of phases (%ld) | Number of instr[0] (%ld) | Total Number of instr  (%ld) | Total offload instr (%ld)", zinfo->numPhases, zinfo->cores[0]->getInstrs(), totalInstrs, totalOffloadInstrs);
...
VOID EndOfPhaseActions() {
    ...
    //cout << "Phase: " << zinfo->numPhases << " - Memory Requests: " << zinfo->num_dram_requests << endl;
    dram_requests << zinfo->numPhases << "," << zinfo->num_dram_requests << "," << offloaded_region << endl;

    zinfo->num_dram_requests = 0;
...
VOID Trace(TRACE trace, VOID *v) {
    if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
        ...
        for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
            BblInfo* bblInfo = Decoder::decodeBbl(bbl, zinfo->oooDecode, zinfo->acceleratorDecode);
...
VOID VdsoInstrument(INS ins) {
    ...
    if (unlikely(insAddr >= vdsoStart && insAddr < vdsoEnd)) {
        if (vdsoEntryMap.find(insAddr) != vdsoEntryMap.end()) {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) VdsoRetPoint, IARG_THREAD_ID, IARG_REG_REFERENCE, REG_RAX, IARG_END);    }*/
...
int main(int argc, char *argv[]) {
    ...
    cout << get_application_name() << endl;
    string tmp = get_application_name() + ".dramRequestsPerPhase";
    dram_requests.open(tmp);
    dram_requests << "Phase,DRAM Requests,Offloded \n";
